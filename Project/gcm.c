...
static int gcm_gen_table(mbedtls_gcm_context *ctx)
{
    ...
    uint64_zero(&ctx->HH[0]);
    uint64_zero(&ctx->HL[0]);

    for (i = 4; i > 0; i >>= 1) {
        uint32_t T = (vl.low & 1) * 0xe1000000U;
        uint64_t temp1 = uint64_shift_left(vh, 63);
        uint64_t temp2 = uint64_shift_right(vl, 1);
        vl  = uint64_or(temp1, temp2);
        temp1 = uint64_shift_right(vh, 1);
        temp2 = uint64_from_uint32_high(T);
        vh  = uint64_xor(temp1, temp2);
    ...
    for (i = 2; i <= 8; i *= 2) {
        ...
        vl = *HiL;
        for (j = 1; j < i; j++) {
            HiH[j] = uint64_xor(vh, ctx->HH[j]);
            HiL[j] = uint64_xor(vl, ctx->HL[j]);
        }
    }
                     ...
                     unsigned char output[16])
{
    ...
    for (i = 15; i >= 0; i--) {
        ...
        if (i != 15) {
            rem = (unsigned char)(zl.low & 0xf);
            temp1 = uint64_shift_left(zh, 60);
            temp2 = uint64_shift_right(zl, 4);
            zl = uint64_or(temp1, temp2);
            zh = uint64_shift_right(zh, 4);
            //temp1 = uint64_from_uint16_shifted48(last4[rem]);
            zh = uint64_xor(zh, last4[rem]);
            zh = uint64_xor(zh, ctx->HH[lo]);
            zl = uint64_xor(zl, ctx->HL[lo]);
        ...
        zh = uint64_shift_right(zh, 4);
        zh = uint64_xor(zh, last4[rem]);
        zh = uint64_xor(zh, ctx->HH[hi]);
        zl = uint64_xor(zl, ctx->HL[hi]);
    }

    MBEDTLS_PUT_UINT32_BE(zh.high, output, 0);
    MBEDTLS_PUT_UINT32_BE(zh.low, output, 4);
    MBEDTLS_PUT_UINT32_BE(zl.high, output, 8);
    MBEDTLS_PUT_UINT32_BE(zl.low, output, 12);
}
                       ...
                       size_t add_len)
{
    ...
    /* Since we don't have 64-bit types we're just doing this */
    if (iv_len == 0 ) {
        return MBEDTLS_ERR_GCM_BAD_INPUT;
    }
    ...
    if (iv_len == 12) {
        ...
        iv_bits.high = 0;
        iv_bits.low = iv_len * 8;
        MBEDTLS_PUT_UINT64_BE(iv_bits.high, work_buf, 8);
                       ...
                       unsigned char *output)
{
    ...
    ctx->len = uint64_add_size_t(ctx->len, length);
                       ...
                       size_t tag_len)
{
    ...
    orig_len = uint64_multiply_by_8(ctx->len);
    orig_add_len = uint64_multiply_by_8(ctx->add_len);
	...
	if(uint64_is_non_zero(orig_len) || uint64_is_non_zero(orig_add_len)){
        memset(work_buf, 0x00, 16);

        MBEDTLS_PUT_UINT32_BE(orig_add_len.high, work_buf, 0);
        MBEDTLS_PUT_UINT32_BE(orig_add_len.low, work_buf, 4);
        MBEDTLS_PUT_UINT32_BE(orig_len.high, work_buf, 8);
        MBEDTLS_PUT_UINT32_BE(orig_len.low, work_buf, 12);
