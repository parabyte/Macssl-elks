                                              ...
                                              size_t len)
{
    ...
    for (p = buf + 2; p < end; p += 2) {
        ...
        if ((sig_cur = mbedtls_ssl_pk_alg_from_sig(p[1])) == MBEDTLS_PK_NONE) {
        	sprintf(debug_str_srv, "client hello v3, signature_algorithm ext"
                                      " unknown sig alg encoding %d", p[1]);
            MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
            continue;
        }
...
#endif
			sprintf(debug_str_srv, "point format selected: %d", p[0]);
            MBEDTLS_SSL_DEBUG_MSG(4, debug_str_srv);
            return 0;
        }
                                 ...
                                 const mbedtls_ssl_ciphersuite_t **ciphersuite_info)
{
    ...
    if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3) {
        ...
        if (sig_type != MBEDTLS_PK_NONE &&
            ...
            sprintf(debug_str_srv, "ciphersuite mismatch: no suitable hash algorithm "
...
read_record_header:
     ...
     *     3  .   4   message length
     */
     sprintf(debug_str_srv, "client hello v3, message type: %d",
                              buf[0]);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
	...
	sprintf(debug_str_srv, "client hello v3, message len.: %d",
                              (ssl->in_len[0] << 8) | ssl->in_len[1]);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);

	sprintf(debug_str_srv, "client hello v3, protocol version: [%d:%d]",
                              buf[1], buf[2]);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
	...
	sprintf(debug_str_srv, "client hello v3, handshake type: %d", buf[0]);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
	...
	sprintf(debug_str_srv, "client hello v3, handshake len.: %d",
                              (buf[1] << 16) | (buf[2] << 8) | buf[3]);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);

    if (buf[1] != 0) {
    	sprintf(debug_str_srv, "bad client hello message: %u != 0",
                                  (unsigned) buf[1]);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str_srv);
        return MBEDTLS_ERR_SSL_BAD_HS_CLIENT_HELLO;
    }
    /* We don't support fragmentation of ClientHello (yet?) */
    if (msg_len != mbedtls_ssl_hs_hdr_len(ssl) + ((buf[2] << 8) | buf[3])) {
    	sprintf(debug_str_srv, "bad client hello message: %u != %u + %u",
                                  (unsigned) msg_len,
    ...
    if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
        ...
        if (ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_IN_PROGRESS) {
            ...
            if (cli_msg_seq != ssl->handshake->in_msg_seq) {
            	sprintf(debug_str_srv, "bad client hello message_seq: "
    ...
    if (ssl->major_ver < ssl->conf->min_major_ver ||
        ...
        sprintf(debug_str_srv, "client only supports ssl smaller than minimum"
                                  " [%d:%d] < [%d:%d]",
                                  ssl->major_ver, ssl->minor_ver,
                                  ssl->conf->min_major_ver, ssl->conf->min_minor_ver);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str_srv);
    ...
    while (ext_len != 0) {
        ...
        switch (ext_id) {
          ...
          MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED */
            ...
            default:
            	sprintf(debug_str_srv, "unknown extension found: %u (ignoring)",
                                          ext_id);
                MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
        }
    ...
    if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3) {
        ...
        if (sig_alg != MBEDTLS_PK_NONE) {
            mbedtls_md_type_t md_alg = mbedtls_ssl_sig_hash_set_find(&ssl->handshake->hash_algs,
                                                                     sig_alg);
            sprintf(debug_str_srv, "client hello v3, signature_algorithm ext: %d",
                                      mbedtls_ssl_hash_from_md_alg(md_alg));
            MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
        } else {
        	sprintf(debug_str_srv, "no hash algorithm for signature algorithm "
...
static int ssl_write_server_hello(mbedtls_ssl_context *ssl)
{
	...
	sprintf(debug_str_srv, "server hello, chosen version: [%d:%d]",
                              buf[4], buf[5]);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
	...
	sprintf(debug_str_srv, "server hello, current time: %" MBEDTLS_PRINTF_LONGLONG,
                              (long long) t);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
#else
    ...
    *p++ = (unsigned char) ssl->session_negotiate->id_len;
	...
	sprintf(debug_str_srv, "server hello, session id len.: %" MBEDTLS_PRINTF_SIZET, n);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
    MBEDTLS_SSL_DEBUG_BUF(3,   "server hello, session id", buf + 39, n);
    sprintf(debug_str_srv, "%s session has been resumed",
    ...
    *p++ = MBEDTLS_BYTE_0(ssl->session_negotiate->compression);
	...
	sprintf(debug_str_srv, "server hello, chosen ciphersuite: %s",
                              mbedtls_ssl_get_ciphersuite_name(ssl->session_negotiate->ciphersuite));
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
    sprintf(debug_str_srv, "server hello, compress alg.: 0x%02X",
	...
	sprintf(debug_str_srv, "server hello, total extension length: %" MBEDTLS_PRINTF_SIZET,
                              ext_len);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
...
curve_matching_done:
    ...
    if (mbedtls_ssl_ciphersuite_uses_server_signature(ciphersuite_info)) {
		...
		sprintf(debug_str_srv, "pick hash algorithm %u for signing", (unsigned) md_alg);
        MBEDTLS_SSL_DEBUG_MSG(3, debug_str_srv);
...
int mbedtls_ssl_handshake_server_step(mbedtls_ssl_context *ssl)
{
	...
	sprintf(debug_str_srv, "server state: %d", ssl->state);
    MBEDTLS_SSL_DEBUG_MSG(2, debug_str_srv);
    ...
    switch (ssl->state) {
        ...
        default:
        	sprintf(debug_str_srv, "invalid state %d", ssl->state);
            MBEDTLS_SSL_DEBUG_MSG(1, debug_str_srv);
            return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
