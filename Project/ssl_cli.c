                                  ...
                                  size_t *olen)
{
    ...
    *olen = 0;
	...
	sprintf(debug_str_cli, "client hello, adding server name extension: %s", ssl->hostname);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);
...
static int ssl_write_client_hello(mbedtls_ssl_context *ssl)
{
	...
	sprintf(debug_str_cli, "client hello, max version: [%d:%d]",
                              buf[4], buf[5]);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);
    ...
    *p++ = (unsigned char) n;
	...
	sprintf(debug_str_cli, "client hello, session id len.: %" MBEDTLS_PRINTF_SIZET, n);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);
    ...
    for (i = 0; ciphersuites[i] != 0; i++) {
		...
		sprintf(debug_str_cli, "client hello, add ciphersuite: %#04x (%s)",
	...
	sprintf(debug_str_cli, "client hello, got %" MBEDTLS_PRINTF_SIZET
    ...
    *q++ = (unsigned char) (n << 1);
    ...
    if (offer_compress) {
    	sprintf(debug_str_cli, "client hello, compress len.: %d", 2);
        MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);
        sprintf(debug_str_cli, "client hello, compress alg.: %d %d",
        ...
        *p++ = MBEDTLS_SSL_COMPRESS_NULL;
    	...
    	sprintf(debug_str_cli, "client hello, compress len.: %d", 1);
        MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);
        sprintf(debug_str_cli, "client hello, compress alg.: %d",
                                  MBEDTLS_SSL_COMPRESS_NULL);
        MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);
	...
	sprintf(debug_str_cli, "client hello, total extension length: %" MBEDTLS_PRINTF_SIZET,
                              ext_len);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);
...
#endif
			sprintf(debug_str_cli, "point format selected: %d", p[0]);
            MBEDTLS_SSL_DEBUG_MSG(4, debug_str_cli);
            return 0;
        }
...
static int ssl_parse_server_hello(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->major_ver < ssl->conf->min_major_ver ||
        ...
        sprintf(debug_str_cli, "server version out of bounds -  min: [%d:%d], server: [%d:%d], max: [%d:%d]",
                                  ssl->conf->min_major_ver,
                                  ssl->conf->min_minor_ver,
                                  ssl->major_ver, ssl->minor_ver,
                                  ssl->conf->max_major_ver,
                                  ssl->conf->max_minor_ver);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str_cli);
	...
	sprintf(debug_str_cli, "server hello, current time: %lu",
    ...
    if (is_compression_bad(ssl, comp)) {
    	sprintf(debug_str_cli, "server hello, bad compression: %d", comp);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str_cli);
        mbedtls_ssl_send_alert_message(
            ssl,
            MBEDTLS_SSL_ALERT_LEVEL_FATAL,
    ...
    if (ssl->handshake->ciphersuite_info == NULL) {
        sprintf(debug_str_cli, "ciphersuite info for %04x not found", (unsigned int) i);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str_cli);
        mbedtls_ssl_send_alert_message(ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL,
	...
	sprintf(debug_str_cli, "server hello, session id len.: %" MBEDTLS_PRINTF_SIZET, n);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);
	...
	sprintf(debug_str_cli, "%s session has been resumed",
                              ssl->handshake->resume ? "a" : "no");
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);

	sprintf(debug_str_cli, "server hello, chosen ciphersuite: %04x", (unsigned) i);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);
    sprintf(debug_str_cli, "server hello, compress alg.: %d",
                              buf[37 + n]);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);
	...
	sprintf(debug_str_cli, "server hello, chosen ciphersuite: %s", suite_info->name);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_cli);
	...
	sprintf(debug_str_cli, "server hello, total extension length: %" MBEDTLS_PRINTF_SIZET,
                           ext_len);
    MBEDTLS_SSL_DEBUG_MSG(2, debug_str_cli);
    ...
    while (ext_len) {
        ...
        switch (ext_id) {
          ...
          MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED */
            ...
            default:
            	sprintf(debug_str_cli, "unknown extension found: %u (ignoring)", ext_id);
                MBEDTLS_SSL_DEBUG_MSG(3,debug_str_cli);
                                      ...
                                      unsigned char *end)
{
    ...
    if (dhm_actual_bitlen < ssl->conf->dhm_min_bitlen) {
    	sprintf(debug_str_cli, "DHM prime too short: %" MBEDTLS_PRINTF_SIZET " < %u",
                                  dhm_actual_bitlen,
...
static int ssl_check_server_ecdh_params(const mbedtls_ssl_context *ssl)
{
	...
	sprintf(debug_str_cli, "ECDH curve: %s", curve_info->name);
    MBEDTLS_SSL_DEBUG_MSG(2, debug_str_cli);
                                         ...
                                         mbedtls_pk_type_t *pk_alg)
{
    ...
    *pk_alg = MBEDTLS_PK_NONE;
    ...
    if ((*md_alg = mbedtls_ssl_md_alg_from_hash((*p)[0]))
        == MBEDTLS_MD_NONE) {
        sprintf(debug_str_cli, "Server used unsupported HashAlgorithm %d", *(p)[0]);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str_cli);
        return MBEDTLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE;
    }
    ...
    if ((*pk_alg = mbedtls_ssl_pk_alg_from_sig((*p)[1]))
        == MBEDTLS_PK_NONE) {
        sprintf(debug_str_cli, "server used unsupported SignatureAlgorithm %d", (*p)[1]);
        MBEDTLS_SSL_DEBUG_MSG(1,debug_str_cli);
        return MBEDTLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE;
    }
    ...
    if (mbedtls_ssl_check_sig_hash(ssl, *md_alg) != 0) {
    	sprintf(debug_str_cli, "server used HashAlgorithm %d that was not offered", *(p)[0]);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str_cli);
                              
        return MBEDTLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE;
    }

	sprintf(debug_str_cli, "Server used SignatureAlgorithm %d",
                              (*p)[1]);
    MBEDTLS_SSL_DEBUG_MSG(2, debug_str_cli);
    sprintf(debug_str_cli, "Server used HashAlgorithm %d",
                              (*p)[0]);
    MBEDTLS_SSL_DEBUG_MSG(2, debug_str_cli);
    *p += 2;
...
static int ssl_parse_certificate_request(mbedtls_ssl_context *ssl)
{
	...
	sprintf(debug_str_cli, "got %s certificate request",
    ...
    if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3) {
        ...
        for (i = 0; i < sig_alg_len; i += 2) {
        	sprintf(debug_str_cli, "Supported Signature Algorithm found: %d,%d",
...
int mbedtls_ssl_handshake_client_step(mbedtls_ssl_context *ssl)
{
	...
	sprintf(debug_str_cli, "client state: %d", ssl->state);
    MBEDTLS_SSL_DEBUG_MSG(2, debug_str_cli);
    ...
    switch (ssl->state) {
        ...
        default:
        	sprintf(debug_str_cli, "invalid state %d", ssl->state);
            MBEDTLS_SSL_DEBUG_MSG(1, debug_str_cli);
            return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
