...
void mbedtls_ssl_set_timer(mbedtls_ssl_context *ssl, uint32_t millisecs)
{
	...
	sprintf(debug_str_msg, "set_timer to %d ms", (int) millisecs);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_msg);
                            ...
                            void *p_rng)
{
    ...
    if (rec->data_len > MBEDTLS_SSL_OUT_CONTENT_LEN) {
    	sprintf(debug_str_msg, "Record content %" MBEDTLS_PRINTF_SIZET
                                  " too large, maximum %" MBEDTLS_PRINTF_SIZET,
                                  rec->data_len,
                                  (size_t) MBEDTLS_SSL_OUT_CONTENT_LEN);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str_msg);
...
hmac_failed_etm_disabled:
    ...
    if (mode == MBEDTLS_MODE_GCM ||
        ...
        MBEDTLS_SSL_DEBUG_BUF(4, "additional data used for AEAD",
                              add_data, add_data_len);
        sprintf(debug_str_msg, "before encrypt: msglen = %" MBEDTLS_PRINTF_SIZET ", "
    ...
    if (mode == MBEDTLS_MODE_GCM ||
        ...
        if (ssl_transform_aead_dynamic_iv_is_explicit(transform) == 1) {
            if (rec->data_len < dynamic_iv_len) {
            	sprintf(debug_str_msg, "msglen (%" MBEDTLS_PRINTF_SIZET
...
#endif
    {
    	sprintf(debug_str_msg, "in_left: %" MBEDTLS_PRINTF_SIZET
                                  ", nb_want: %" MBEDTLS_PRINTF_SIZET,
                                  ssl->in_left, nb_want);
        MBEDTLS_SSL_DEBUG_MSG(2, debug_str_msg);
        ...
        while (ssl->in_left < nb_want) {
            ...
            if ((size_t) ret > len
                ...
                sprintf(debug_str_msg, "f_recv returned %d bytes but only %" MBEDTLS_PRINTF_SIZET
                                       " were requested",
                                       ret, len);
                MBEDTLS_SSL_DEBUG_MSG(1, debug_str_msg);
                return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
...
int mbedtls_ssl_flush_output(mbedtls_ssl_context *ssl)
{
    ...
    while (ssl->out_left > 0) {
    	sprintf(debug_str_msg, "message length: %" MBEDTLS_PRINTF_SIZET
        ...
        if ((size_t) ret > ssl->out_left
            ...
            sprintf(debug_str_msg, "f_send returned %d bytes but only %" MBEDTLS_PRINTF_SIZET
                                   " bytes were sent",
                                   ret, ssl->out_left);
            MBEDTLS_SSL_DEBUG_MSG(1, debug_str_msg);
            return MBEDTLS_ERR_SSL_INTERNAL_ERROR;
...
int mbedtls_ssl_write_handshake_msg(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->out_msglen > MBEDTLS_SSL_OUT_CONTENT_LEN) {
    	sprintf(debug_str_msg, "Record too large: "
     ...
     */
    if (ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE) {
        ssl->out_msg[1] = MBEDTLS_BYTE_2((uint32_t) hs_len);
        ssl->out_msg[2] = MBEDTLS_BYTE_1((uint32_t) hs_len);
        ssl->out_msg[3] = MBEDTLS_BYTE_0((uint32_t) hs_len);
...
int mbedtls_ssl_write_record(mbedtls_ssl_context *ssl, uint8_t force_flush)
{
    ...
    if (!done) {
		...
		sprintf(debug_str_msg, "output record: msgtype = %u, "
...
int mbedtls_ssl_prepare_handshake_record(mbedtls_ssl_context *ssl)
{
    if (ssl->in_msglen < mbedtls_ssl_hs_hdr_len(ssl)) {
    	sprintf(debug_str_msg, "handshake message too short: %" MBEDTLS_PRINTF_SIZET,
                                  ssl->in_msglen);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str_msg);
        return MBEDTLS_ERR_SSL_INVALID_RECORD;
    }
	...
	sprintf(debug_str_msg, "handshake message: msglen ="
                                   ...
                                   mbedtls_record *rec)
{
    ...
    if (len < rec_hdr_len_offset + rec_hdr_len_len) {
    	sprintf(debug_str_msg, "datagram of length %u too small to hold DTLS record header of length %u",
                                  (unsigned) len,
...
#endif /* MBEDTLS_SSL_DTLS_CONNECTION_ID */
    {
        if (ssl_check_record_type(rec->type)) {
        	sprintf(debug_str_msg, "unknown record type %u",
                                      (unsigned) rec->type);
            MBEDTLS_SSL_DEBUG_MSG(1, debug_str_msg);
            return MBEDTLS_ERR_SSL_INVALID_RECORD;
        }
    ...
    if (major_ver != ssl->major_ver) {
    	sprintf(debug_str_msg, "major version mismatch: got %u, expected %u",
                                  (unsigned) major_ver,
    ...
    if (minor_ver > ssl->conf->max_minor_ver) {
    	sprintf(debug_str_msg, "minor version mismatch: got %u, expected max %u",
                                  (unsigned) minor_ver,
	...
	sprintf(debug_str_msg, "input record: msgtype = %u, "
                                      ...
                                      mbedtls_record *rec)
{
    ...
    if (!done && ssl->transform_in != NULL) {
        ...
        if (old_msg_type != rec->type) {
        	sprintf(debug_str_msg, "record type after decrypt (before %d): %d",
                                      old_msg_type, rec->type);
            MBEDTLS_SSL_DEBUG_MSG(4, debug_str_msg);
        }
        ...
        if (rec->data_len == 0) {
            ...
            if (ssl->minor_ver == MBEDTLS_SSL_MINOR_VERSION_3
                ...
                sprintf(debug_str_msg, "invalid zero-length message type: %d", ssl->in_msgtype);
                MBEDTLS_SSL_DEBUG_MSG(1, debug_str_msg);
                return MBEDTLS_ERR_SSL_INVALID_RECORD;
            }
...
int mbedtls_ssl_handle_message_type(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->in_msgtype == MBEDTLS_SSL_MSG_CHANGE_CIPHER_SPEC) {
        if (ssl->in_msglen != 1) {
        	sprintf(debug_str_msg, "invalid CCS message, len: %" MBEDTLS_PRINTF_SIZET,
                                      ssl->in_msglen);
            MBEDTLS_SSL_DEBUG_MSG(1, debug_str_msg);
            return MBEDTLS_ERR_SSL_INVALID_RECORD;
        }
        ...
        if (ssl->in_msg[0] != 1) {
        	sprintf(debug_str_msg, "invalid CCS message, content: %02x",
                                      ssl->in_msg[0]);
            MBEDTLS_SSL_DEBUG_MSG(1, debug_str_msg);
            return MBEDTLS_ERR_SSL_INVALID_RECORD;
        }
    ...
    if (ssl->in_msgtype == MBEDTLS_SSL_MSG_ALERT) {
        if (ssl->in_msglen != 2) {
            ...
            sprintf(debug_str_msg, "invalid alert message, len: %" MBEDTLS_PRINTF_SIZET,
                                      ssl->in_msglen);
            MBEDTLS_SSL_DEBUG_MSG(1, debug_str_msg);
            return MBEDTLS_ERR_SSL_INVALID_RECORD;
        }
		sprintf(debug_str_msg, "got an alert message, type: [%u:%u]",
        ...
        if (ssl->in_msg[0] == MBEDTLS_SSL_ALERT_LEVEL_FATAL) {
        	sprintf(debug_str_msg, "is a fatal alert message (msg %d)",
                                      ssl->in_msg[1]);
            MBEDTLS_SSL_DEBUG_MSG(1, debug_str_msg);
                                   ...
                                   unsigned char message)
{
    ...
    sprintf(debug_str_msg, "send alert level=%u message=%u", level, message);
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str_msg);
