                                  ...
                                  mbedtls_ssl_context *ssl)
{
    ...
    if (ciphersuite_info == NULL) {
    	sprintf(debug_str, "ciphersuite info for %d not found", ciphersuite);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str);
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }

    cipher_info = mbedtls_cipher_info_from_type(ciphersuite_info->cipher);
    if (cipher_info == NULL) {
    	sprintf(debug_str, "cipher info for %u not found", ciphersuite_info->cipher);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str);
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }
    ...
    if (md_info == NULL) {
    	sprintf(debug_str, "mbedtls_md info for %u not found",
	...
	sprintf(debug_str, "ciphersuite = %s",
                              mbedtls_ssl_get_ciphersuite_name(ciphersuite));
    MBEDTLS_SSL_DEBUG_MSG(3, debug_str);
	...
	sprintf(debug_str, "keylen: %u, minlen: %u, ivlen: %u, maclen: %u",
                              (unsigned) keylen,
...
int mbedtls_ssl_write_certificate(mbedtls_ssl_context *ssl)
{
    ...
    while (crt != NULL) {
        ...
        if (n > MBEDTLS_SSL_OUT_CONTENT_LEN - 3 - i) {
        	sprintf(debug_str, "certificate too large, %" MBEDTLS_PRINTF_SIZET
        ...
        ssl->out_msg[i] = MBEDTLS_BYTE_2((uint32_t) n);
        ssl->out_msg[i + 1] = MBEDTLS_BYTE_1((uint32_t) n);
        ssl->out_msg[i + 2] = MBEDTLS_BYTE_0((uint32_t) n);

        i += 3; memcpy(ssl->out_msg + i, crt->raw.p, n);
        i += n; crt = crt->next;
    }

    ssl->out_msg[4]  = MBEDTLS_BYTE_2((uint32_t) (i - 7));
    ssl->out_msg[5]  = MBEDTLS_BYTE_1((uint32_t) (i - 7));
    ssl->out_msg[6]  = MBEDTLS_BYTE_0((uint32_t) (i - 7));
                                        ...
                                        void *rs_ctx)
{
    ...
    if (ssl->session_negotiate->verify_result != 0) {
    	sprintf(debug_str, "! Certificate verification flags %08x",
                                        ...
                                        unsigned char *start, size_t len)
{
    ...
    if (ssl->session_negotiate->peer_cert_digest == NULL) {
    	sprintf(debug_str, "alloc(%d bytes) failed",
...
int mbedtls_ssl_parse_certificate(mbedtls_ssl_context *ssl)
{
    ...
    if (chain == NULL) {
    	sprintf(debug_str, "alloc(%" MBEDTLS_PRINTF_SIZET " bytes) failed",
                      ...
                      const mbedtls_ssl_config *conf)
{
    ...
    if (ssl->in_buf == NULL) {
    	sprintf(debug_str, "alloc(%" MBEDTLS_PRINTF_SIZET " bytes) failed", in_buf_len);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str);
        ret = MBEDTLS_ERR_SSL_ALLOC_FAILED;
        goto error;
    }
    ...
    if (ssl->out_buf == NULL) {
    	sprintf(debug_str, "alloc(%" MBEDTLS_PRINTF_SIZET " bytes) failed", out_buf_len);
        MBEDTLS_SSL_DEBUG_MSG(1, debug_str);
        ret = MBEDTLS_ERR_SSL_ALLOC_FAILED;
        goto error;
    }
